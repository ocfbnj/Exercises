...
void reslove(int count, int i, int j, int x, int y, const std::string& str) {
    if (count < 7 && i >= 0 && i < n && j >= 0 && j < n && lines[i][j] == des[count]) {
        ...
        reslove(count + 1, i + 1, j, 1, 0, temp);
        reslove(count + 1, i - 1, j, -1, 0, temp);
        reslove(count + 1, i, j + 1, 0, 1, temp);
        reslove(count + 1, i, j - 1, 0, -1, temp);
        reslove(count + 1, i + 1, j + 1, 1, 1, temp);
        reslove(count + 1, i + 1, j - 1, 1, -1, temp);
        reslove(count + 1, i - 1, j + 1, -1, 1, temp);
        reslove(count + 1, i - 1, j - 1, -1, -1, temp);
    }
