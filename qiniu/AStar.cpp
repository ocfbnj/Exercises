...
void AStar::__try_move(__Position* prev, std::unique_ptr<__Position> cur) {
	...
	else if (auto it = std::find_if(_open_list.begin(), _open_list.end(), pred); 
		it != _open_list.end()) {
...
int main() {
	{
		AStar astar(2, 2, 2, 7);
		astar.set_map((char*)map, 5, 11);
		auto ret = astar.get_route();
		for (auto&& [x, y] : ret) {
			std::cout << "(" << x << ", " << y << ")\n";
		}
	}

	std::cout << "ctor: " << AStar::__Position::ctor << std::endl;
	std::cout << "dtor: " << AStar::__Position::dtor << std::endl;
}
